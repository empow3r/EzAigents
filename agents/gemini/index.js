...
const ROLE = process.env.ROLE || 'analysis';

(async () => {
  console.log(`🔍 Gemini Agent [${ROLE}] starting...`);
  while (true) {
    ...
    try {
      ...
      const outPath = path.join('src/output', `analysis__${file.replace(/\//g, '__')}`);
      ...
      const taskInsight = `Successfully ${isNewFile ? 'created' : 'modified'} ${file} - gemini specialization`;
      memoryManager.saveLearning(taskInsight, 'gemini_tasks');
      
      await redis.lrem(`processing:${MODEL}`, 1, job);
      console.log(`✅ GEMINI [${ROLE}] updated: ${file}`);
      
      // Clear context after successful completion to save tokens
      await memoryManager.clearContextAfterTask(taskId);
    } catch (e) {
      console.error(`❌ GEMINI [${ROLE}] error on ${file}:`, e.message);
